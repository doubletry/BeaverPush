...
.NOTES
    前置条件：
      - Python >= 3.12（已通过 Poetry 管理）
      - poetry install 已执行
      - Nuitka 已安装（已在 pyproject.toml 中声明）
#>
...
Write-Host "  BeaverPush Nuitka 编译打包" -ForegroundColor Blue
Write-Host "========================================" -ForegroundColor Blue
Write-Host ""
...
if (-not (Test-Path $IssFile)) {
    Write-Host "[SKIP] 未找到 installer.iss, 跳过安装包打包" -ForegroundColor Yellow
    exit 0
}
...
if (-not $Iscc) {
    Write-Host ""
    Write-Host "[SKIP] 未找到 Inno Setup (iscc.exe), 跳过安装包打包" -ForegroundColor Yellow
    Write-Host "[TIP]  请从 https://jrsoftware.org/isinfo.php 下载安装 Inno Setup 6" -ForegroundColor Yellow
    Write-Host "[TIP]  安装后重新运行此脚本即可自动生成安装包" -ForegroundColor Yellow
    exit 0
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Blue
Write-Host "  Inno Setup 安装包打包" -ForegroundColor Blue
Write-Host "========================================" -ForegroundColor Blue
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "[ERROR] 安装包打包失败, 退出码: $LASTEXITCODE" -ForegroundColor Red
    exit $LASTEXITCODE
}
