# BeaverPush — 河狸推流（多路 RTSP 推流客户端）

[English](README.md)

基于 **PySide6 + MVC 架构** 的 Windows 多路 RTSP 推流桌面客户端。

## 功能特性

- 🎥 **6 种视频源：** 本地视频、摄像头、RTSP 拉流转推、屏幕捕获、窗口捕获、海康工业相机
- 📡 **多路同时推流**，每路独立控制启停
- 🔐 **v2 认证推流：** 支持 window-to-web v2 的用户名 + 授权码认证，推流路径为 `用户名/设备名/通道名` 三级结构
- 🎨 **Catppuccin Mocha** 暗色主题
- 🔧 **编码参数可配：** 编码器（h264/h265/NVENC）、分辨率、帧率、码率
- 💾 **配置自动持久化**（JSON 格式）
- 👁️ **ffplay 实时预览**
- 🖥️ **系统托盘** 最小化
- 🔒 **服务器地址锁定**，防止误修改
- 🔄 **本地视频循环播放**
- 🖱️ **可编辑通道名称**，点击标题即可修改
- 🔑 **自动检测设备名**，使用主板 UUID
- ✅ **输入校验**，用户名、设备名和流名称仅允许 ASCII 安全字符
- 🔄 **自动重连**，RTSP 源、海康相机源支持可配置的重连间隔与最大重试次数
- 💾 **连接测试成功后自动保存配置**
- 🚀 **开机自启动**（Windows）：勾选后随系统启动并自动恢复上次正在推流的通道

## 多路启动与 RTSP 认证说明

- 自动启动、全部启动、手动启动和断线重连共用一个全局启动队列。只有当前通道进入“推流中”或明确失败后，下一路才开始，避免多路 FFmpeg 同时进行认证与编码器初始化。
- 普通推流不会额外向目标服务器发布 `__connection_test__` 测试流；工具栏中的显式“测试连接”仍保留此检查。
- 对需要认证的 RTSP 服务器，每个新连接可能先发送一次无凭据请求，收到 `401 Unauthorized` 后再携带凭据重试。这是 RTSP 的认证挑战流程，不表示软件遗漏了配置的凭据。
- 停止或退出时会统一回收应用创建的 FFmpeg/ffplay 进程；若正常终止超时，会升级为终止整个进程树并强制结束。

## 下载安装

从 [GitHub Releases](https://github.com/doubletry/BeaverPush/releases) 下载最新安装包。安装包已内置 FFmpeg，无需额外配置。

## 开发环境搭建

### 前置依赖

- **Python** ≥ 3.12
- **FFmpeg** / **ffprobe** / **ffplay** 在 `PATH` 中（或放置在 `ffmpeg/` 子目录下）
- **uv** 包管理器

### 安装与运行

```bash
# 安装依赖
uv sync

# 运行应用
uv run beaverpush
# 或
uv run python -m beaverpush.main
```

### 运行测试

```bash
uv run pytest
```

### 从源码构建

构建独立可执行文件和 Windows 安装包：

```powershell
# 编译 + 打包安装程序（需要安装 Inno Setup 6）
.\build.ps1 -Version "1.0.0"
```

构建脚本使用 **Nuitka** 编译独立可执行文件（`dist/main.dist/BeaverPush.exe`），并通过 **Inno Setup** 生成安装包（`dist/BeaverPushSetup.exe`）。
若要在本地生成安装包，还需要预先把 shared FFmpeg 构建放到项目 `ffmpeg/` 目录中（包含 `ffmpeg.exe`、`ffplay.exe`、`ffprobe.exe` 和所需的 `*.dll`）。CI 会在调用 `build.ps1` 前自动下载并整理这些文件。

## 使用说明

1. 填写 RTSP 服务器地址（如 `rtsp://192.168.1.100:8554`）
2. 填写 **用户名**（window-to-web 上的账户名）和 **授权码**（在 window-to-web 网页端生成的 API Key）
3. 设置 **设备名**，用于区分同一用户的不同推流设备（留空时自动使用主板 UUID）
4. 点击 **添加通道** 创建推流通道
5. 选择视频源类型并配置参数
6. 点击 **开始推流**（流名称留空时自动分配 `stream1`、`stream2` 等）

推流路径结构为三级：`{用户名}/{设备名}/{通道名}`，例如 `alice/pc1/stream1`。

### 视频源类型

| 类型 | 说明 |
|------|------|
| 本地视频 | 推送视频文件（支持循环播放） |
| 摄像头 | 通过 DirectShow 捕获本地摄像头 |
| RTSP 拉流 | 从 RTSP 源拉流并转推 |
| 屏幕捕获 | 捕获显示器/屏幕区域 |
| 窗口捕获 | 捕获指定应用程序窗口 |
| 海康工业相机 | 通过 SN 获取海康工业相机画面 |

### 高级设置

在通道卡片上切换 **高级模式** 可配置：
- **编码器：** libx264、h264_nvenc、hevc_nvenc、copy
- **分辨率：** 宽 × 高（自动调整为偶数）
- **帧率** 和 **码率**（单位固定为 `M`；码率留空则不限制）

## 项目结构

```
src/beaverpush/
├── main.py                      # 应用入口
├── models/
│   ├── config.py                # JSON 配置持久化 (AppConfig, StreamConfig)
│   ├── stream_model.py          # StreamState 推流状态枚举
│   └── source_type_plugin.py    # SourceTypePlugin 协议 + 6 个内置插件
├── views/
│   ├── theme.py                 # Catppuccin Mocha 主题 + QSS
│   ├── stream_card.py           # 推流通道卡片组件
│   └── main_window.py           # 主窗口（工具栏 + 滚动卡片列表）
├── controllers/
│   ├── _utils.py                # 共享工具函数
│   ├── app_controller.py        # 应用生命周期、配置管理、设备枚举
│   ├── stream_controller.py     # 单路推流 FFmpeg 生命周期管理
│   └── reconnect_policy.py      # 重连状态机
└── services/
    ├── device_service.py        # 设备枚举（摄像头/屏幕/窗口）
    ├── ffmpeg_service.py        # 向后兼容包装器（从子模块导入）
    ├── ffmpeg_command.py        # FFmpeg 命令构建 + 错误映射
    ├── ffmpeg_worker.py         # FFmpegWorker QThread 进程管理
    ├── rtsp_url.py              # RTSP URL 构建 + 认证
    ├── codec_registry.py        # 编码器可用性注册表（单例）
    ├── ffmpeg_path.py           # FFmpeg 可执行文件路径解析
    ├── log_service.py           # 基于 Loguru 的日志服务
    └── window_capture.py        # Win32 窗口/屏幕捕获 (PrintWindow/BitBlt)
```

## 架构

```
┌───────────────────────────────────────────────┐
│                     Views                      │
│  MainWindow ◄──── StreamCardView (×N)          │
│  (Qt 信号)        (Qt 信号)                     │
└──────┬──────────────────┬──────────────────────┘
       │                  │
       ▼                  ▼
┌──────────────┐  ┌─────────────────┐
│AppController │  │StreamController │  ← Controllers
│ (全局管理)    │  │ (单路推流)       │
└──────┬───────┘  └────────┬────────┘
       │                   │
       ▼                   ▼
┌───────────────────────────────────────────────┐
│              Models + Services                 │
│  config · stream_model · source_type_plugin    │
│  ffmpeg_command · ffmpeg_worker · rtsp_url     │
│  codec_registry · device_service               │
└───────────────────────────────────────────────┘
```

- **Views** — 仅负责 UI 展示，通过 Qt 信号通知用户操作
- **Controllers** — 连接信号、调用 Services、通过 `set_*` 方法更新视图
- **Services** — 封装纯业务逻辑（FFmpeg 进程、设备枚举、窗口捕获）
- **Models** — 定义数据结构、持久化和插件接口

## CI/CD

推送版本标签（如 `v1.0.0`）时自动触发 GitHub Actions 构建流程：

1. 配置 Python 3.12 + uv 环境
2. 下载打包所用的固定 FFmpeg n8.1 shared 二进制文件
3. 安装 Inno Setup 6
4. 使用 Nuitka 编译并打包安装程序
5. 执行静默安装验证测试

## 许可协议

[MIT](LICENSE)
